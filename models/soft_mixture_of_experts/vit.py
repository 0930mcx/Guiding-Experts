...
from transformer import SoftMoEEncoder, SoftMoEEncoderLayer
...
class ViTWrapper(nn.Module):
    ...
    def forward(self, x: Tensor, masks: Tensor = None, is_front: bool = True,return_features: bool = False):
        ...
        x = self.dropout(x)
        x = self.encoder(x, masks=masks, is_front=is_front)
        aux = None
...
class SoftMoEViT(ViTWrapper):
    def __init__(
        ...
        encoder = SoftMoEEncoder(encoder_layer, num_layers=num_encoder_layers, last_n=last_n)
        super().__init__(
            num_classes=num_classes,
            encoder=encoder,
            image_size=image_size,
